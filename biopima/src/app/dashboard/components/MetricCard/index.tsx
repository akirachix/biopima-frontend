...
export default function MetricCard({ title, value, unit, description, variant }: MetricCardProps) {
  const iconColor = variant === 'methane' ? 'text-green-600' :
                   variant === 'temperature' ? 'text-pink-600' : 'text-orange-600';

  const borderColor = variant === 'methane' ? 'border-green-200' :
                     variant === 'temperature' ? 'border-pink-300' : 'border-orange-300';

  const bgColor = variant === 'methane' ? 'bg-green-100' :
                 variant === 'temperature' ? 'bg-pink-100' : 'bg-orange-100';

  const Icon = variant === 'methane' ? Activity :
              variant === 'temperature' ? Thermometer : Gauge;

  return (
    <div className={`bg-white rounded-3xl p-4 sm:p-5 shadow-lg border ${borderColor}`}>
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1 sm:mt-2">{description}</p>
        </div>
        <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-3xl flex items-center justify-center ${bgColor} flex-shrink-0`}>
